...
- When deploying to the VPS, push committed source changes to GitHub first, then update the server checkout with `git pull --ff-only`, `docker compose pull`, and `docker compose up -d --remove-orphans`. Sync files manually only for first bootstrap or when the server is not yet a Git checkout.
