...
- Runtime containers must not mount application assets, source code, generated client files, logs, CMS storage, caches, or config files. The only normal runtime mounts are database-related files/volumes such as MySQL data, database init dumps, database config, and database backups. Development-only bind mounts are allowed only in an explicit dev compose/service.
- Rebuild and recreate only services affected by a change. For local and production deploys, target the changed image/service names instead of rebuilding or recreating the whole stack.
